...
Start with Explore, then run Planner -> Plan Critic until Critic says no additional plan changes are required. Stop if HITL is required. After approval, delegate implementation, testing, fixing, Type Interface Organizer when applicable, and final review.

After Tester passes, if the repository has `tsconfig.json` or the diff includes TS/TSX files, spawn a subagent with `$type-interface-organizer` injected. It must run report mode first, apply only analyzer-marked conservative edits, write `.pipeline/type-interface-report.md`, write `.pipeline/type-interface-summary.json` when available, and update `.pipeline/changes.md` when edits are applied. If it applies edits, rerun Tester before Final Reviewer.
